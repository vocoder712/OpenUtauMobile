...
namespace OpenUtau.Core.Render {
    ...
    public class RenderPhone {
        ...
        internal RenderPhone(Pipeline.PhraseSource source, Pipeline.PhonemeSource phoneme, int phrasePosition, int noteIndex) {
            position = source.PartPosition + phoneme.Position - phrasePosition;
            duration = phoneme.Duration;
            end = position + duration;
            positionOverridden = phoneme.PositionOverridden;
    ...
    public class RenderPhrase {
        ...
        internal RenderPhrase(Pipeline.PhraseSource source, int phraseStart, int phraseEnd) {
            ...
            Pipeline.NoteSource firstSourceNote = notesOf[phrasePhonemes.First().NoteIndex];
            int scoreOriginTick = source.PartPosition + firstSourceNote.Position;
            int contextStartTick = firstSourceNote.Prev == -1
                ? 0
                : source.PartPosition + notesOf[firstSourceNote.Prev].End;
            contextStartTick = Math.Min(scoreOriginTick, contextStartTick);
            availableLeadingMs = Math.Max(0,
                timeAxis.TickPosToMsPos(scoreOriginTick) - timeAxis.TickPosToMsPos(contextStartTick));
            ...
            phones = phrasePhonemes
                .Select(p => new RenderPhone(source, p, position, noteIndexByNote[p.NoteIndex]))
                .ToArray();

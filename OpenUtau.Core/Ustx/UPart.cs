    ...
    public class UWavePart : UPart {
        ...
        public void Load(UProject project) {
            ...
            Peaks = Task.Run(() => {
                ...
                stopwatch.Stop();
                Log.Information($"音频分片加载完成 {FilePath} {stopwatch.Elapsed}");
                ...
                stopwatch.Stop();
                Log.Information($"音频波形读取完成 {FilePath} {stopwatch.Elapsed}");
                return peaks;
            });
            Peaks.ContinueWith(t => {
                DocManager.Inst.ExecuteCmd(new LoadingNotification(typeof(UWavePart), false, DisplayName));
            });
        }

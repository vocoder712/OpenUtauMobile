...
- Date: 2026-09-11
- Decision: Use the iOS app Documents directory as OpenUtau's writable data root, expose it through iOS file sharing/open-in-place, initialize ONNX Runtime during host startup, and implement audio output with an `AVAudioEngine` source node that pulls the existing 44.1 kHz stereo sample-provider stream.
- Rationale: iOS application bundles are read-only and desktop audio backends are unavailable. Documents provides a user-visible sandbox location for projects and audio, an early `OrtEnv`/`SessionOptions` probe verifies that the native ONNX payload can load independently of a singing engine, and AVAudioEngine supplies the platform-native output path without changing the shared playback pipeline.
- Alternatives considered: Keep the dummy iOS audio backend; use a desktop NAudio backend on iOS; treat successful native linking as sufficient ONNX validation; store user data under the application bundle.
- Impacted areas: iOS host startup, sandbox paths and document sharing, ONNX startup diagnostics, audio playback, and the audio-backend option shown by shared settings UI.
